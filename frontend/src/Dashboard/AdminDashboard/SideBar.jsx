  ...
  return (
    <>
      ...
      <aside
        ...
        border-r border-gray-100 top-0`}
        style={{ pointerEvents: "auto" }}
      >
        ...
        {!isOpen && (
          <div className="px-2 py-2 sm:px-3 sm:py-3 lg:px-4 lg:py-3 border-t border-gray-100">
            <Button
              variant="ghost"
              className="w-full flex items-center justify-center p-2 sm:p-2.5 lg:p-3 text-gray-600 hover:text-red-500 hover:bg-red-50"
              onClick={() => setShowLogoutModal(true)}
              disabled={isLoggingOut}
              title="Logout"
            >
        ...
        {/* Toggle Button - Show on all screen sizes when collapsed */}
        {!isOpen && (
          <button
            onClick={toggleSidebar}
            className="fixed top-4 left-4 p-2 bg-white hover:bg-gray-50 rounded-md shadow-sm border border-gray-100 transition-all duration-300 z-50"
        ...
        {/* Toggle Button - Show on desktop when expanded */}
        {!isMobile && isOpen && (
          <button
            onClick={toggleSidebar}
            className={`fixed top-4 ${
              isOpen ? "left-[220px] sm:left-[248px] lg:left-[280px]" : "left-4"
            } p-2 bg-white hover:bg-gray-50 rounded-md shadow-sm border border-gray-100 transition-all duration-300 z-50`}
